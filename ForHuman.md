...
`myproject-architecture`)というフラットなidになる。
...
- **「詳細は子文書を参照」を本文に書かせない**: `direct_children()` が
  ディレクトリ構造から子トピックを機械的に一覧化し、`doc show` / `doc toc` が自動で出す。
  手書きの導線文は書き忘れると孤立サブトピックになるので、仕組み側で解決している。
- **topic id の衝突**: id はパスの `/` を `-` に潰したものなので、`x-y.md` と `x/y.md`、
  `x.md` と `x/README.md` はそれぞれ同じ id になる。**どちらか一方しか置けない。**
  `doc check` の topic id 重複検出が id 一意性の唯一の担保なので弱めてはいけない。
  この非可逆性を受け入れたのは、階層をパスに寄せる代わりに**文書をフォルダへ移しても
  id が変わらない**——つまり参照を壊さずに再編できる——性質を取ったため。
  階層は `ns_of()` がパスから導く。
